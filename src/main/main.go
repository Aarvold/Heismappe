...
func connectionExists(addr string)bool{
	return connection, exist := onlineElevs[addr]; exist
}

func addNewConnection(addr string, aliveTimeout int){
	...
	for {
		...
		case msg := <-costMsg:
			...
			if costList, exist := rcvList[newOrder]; exist {
				if !costAlreadyRecieved(costList,newCost) {
...
func addNewOrder(rcvList []rcvCost, newOrder rcvOrder){
...
func allCostsRecieved(rcvList []rcvCost,newOrder rcvOrder)bool{
	return len(rcvList[newOrder]) == numOfOnlineElevs
}

func costAlreadyRecieved(newCost rcvCost,costList[]rcvCost){
